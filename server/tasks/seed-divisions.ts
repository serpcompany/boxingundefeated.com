...
export default defineTask({
  ...
  async run() {
    ...
    // Insert each record one‑by‑one. This lets rows omit optional fields without
    // causing mismatched bind parameters.
    let inserted = 0
    for (const division of divisionsData) {
      await useDrizzle().insert(tables.divisions).values(division)
      inserted += 1
    }
